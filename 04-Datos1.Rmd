...
Otra estructura de datos importante son los llamados *factores*. Los factores
generalmente se ven como datos tipo caracter, pero son usado tipicamente para
representar información categórica. Por ejemplo, hagamos un vector de cadena 
...
Vemos que las data.frames se ven como listas "debajo del capot" - esto es porque
en realidad un data.frame es una lista de vectores y factores - para lograr
contener esas columnas que son una mezcla de vectores y factores, la data.frame
necesita algo más flexible que un vector para poner todas las columnas en
una tabla común. En otras palabras, `data.frame` es una lista especial en la
cual todos los vectores tienen que tener la misma longitud.
...
<div class='solution'>
## Solución al Desafio 4
¿Cuál crees que será el resultado de 
...
Porque la matriz es un vector con el añadido de atributos de dimension, 
`length` te da el número total de elementos dentro de la matriz.
</div>
